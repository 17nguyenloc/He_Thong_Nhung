...
void sendTask2(void *arg) {/* keep the status of sending data */
	...
	while (1) {
		...
		/* we delay here so that receiveTask has chance to receive data */
		vTaskDelay(pdMS_TO_TICKS(2000)); //Tiempo de bloqueo
	}
	vTaskDelete( NULL);
}
void sendTask3(void *arg) {/* keep the status of sending data */
	BaseType_t xStatus;
	/* create data to send */
	Data data3;
	/* sender 1 has id is 1 */
	...
	while (1) {
		...
		if (xStatus == pdPASS) {
			/* increase counter of sender 1 */
			data3.counter = data3.counter + 1;
		}
		/* we delay here so that receiveTask has chance to receive data */
		vTaskDelay(pdMS_TO_TICKS(3000)); //Tiempo de bloqueo
	}
	vTaskDelete( NULL);
}
...
void app_main() {
    ...
    xTaskCreatePinnedToCore(sendTask3, "sendTask3", 10000, NULL, 2, NULL, 0); //Crea tarea que lee valores de la cola.
